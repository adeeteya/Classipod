...
desc "Build an unsigned iOS IPA or ad-hoc-signed, unnotarized macOS DMG"
lane :build_apple_release do |options|
  platform = options[:platform].to_s
  unless %w[ios macos].include?(platform)
    UI.user_error!("platform must be ios or macos")
  end
  ...
  sh("cd #{Shellwords.escape(repository)} && " \
     "bash scripts/package_apple_release.sh #{platform}")
end
